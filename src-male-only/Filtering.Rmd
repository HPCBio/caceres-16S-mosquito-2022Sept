...
Here, we will normally prune out sample MB8, which was determined as a significant outlier by the overall compositional analysis and from a prior data run.  However this is already removed with the female samples

<!-- MB4 and MB5 however also have a distinct pattern, and based on later analyses is a significant outlier.  Let's remove. -->
...
23 are not assigned at the phylum rank.  Export the unassigned sequences to a file.
...
writeXStringSet(refseq(unasn), 
                filepath = '../results/unasn-sequences/unassigned.male.fna', 
                format = 'fasta')
```
...
saveRDS(physeq.filtered, 
        '~/research/biotech/young-chae_kim/2021-Fall-Microbiome/results/physeq.filtered.males.RDS')
```
