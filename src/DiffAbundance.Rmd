...
res.test = generateContrastResults(deseq = finalPhyseq.adds,
                              contrast = c("Group", "I", "L"),
                              physeq = physeq.prev,
                              file = "results/DESeq2/Overall-I-vs-L.txt"
)
dim(res.test[which(res.test$padj <= 0.05), ])
```

There are 116 significant ASVs.


Plot at Family-level
```{r }
plotDESeq2ResFamily(res.test)
ggsave("Diff-Abund_Family_I-vs-L", path = "results/DESeq2/", width = 11, height = 8,device = "pdf")
```

Plot at Genus-level
```{r }
plotDESeq2ResGenus(res.test)
ggsave("Diff-Abund_Genus_I-vs-L", path = "results/DESeq2/", width = 12, height = 8, device = "pdf")
```

Plot at Species-level
```{r }
plotDESeq2ResSpecies(res.test)
ggsave("Diff-Abund_Species_I-vs-L", path = "results/DESeq2/", width = 18, height = 10, device = "pdf")
```
