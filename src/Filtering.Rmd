...
Are we focusing on one particular subset of samples?  Here we are, specifically the male samples.
...
```{r }
physeq.filtered <- prune_samples(!(sample_names(physeq.filtered) %in% c("MB5")), physeq.filtered)
physeq.filtered
```
...
How many Phylum are unassigned?

```{r }
table(is.na(tax_table(physeq.filtered)[,'Phylum'])) %>% kable
```

26 are not assigned at the phylum rank.  Export the sequences to a file.
