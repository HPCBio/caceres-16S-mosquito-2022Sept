...
Will remove Infusion samples for now, but can add them back in later.
...
After removing Infusion samples, 571 are 0 count.
...
22 are not assigned at the phylum rank.  Export the unassigned sequences to a file.
...
Pretty reasonable spread. However there is one samples below our standard 10k count limit, so may remove this later.
...
```
Can see that the 50 most abundant ASVs are dominant in water samples, and seeing mostly Arcobacteraceae family in all extraction types.
