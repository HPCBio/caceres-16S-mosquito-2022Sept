---
title: "Kim 16S analysis - Oct 2021"
author: "Chris Fields"
...
Mouse study, split by treatment and sex.  For this we will focus on the male mice.
...
We see a fairly substantial drop from merging, so we're testing using just R1
...
We know all have at least a 'Bacteria' domain-level assignment.  42 are not assigned at the phylum rank.  Export the sequences to a file.
...
# remove mitochondria
physeq.filtered <- subset_taxa(physeq, is.na(Family) | Family != 'Mitochondria')
...
```{r }
physeq.filtered <- prune_samples(!(sample_names(physeq.filtered) %in% c("MB8")), physeq.filtered)
# physeq.filtered
```
