...
plots <- lapply(ranks, function (x) { 
  ...
  p1 <- p + theme(legend.key.size = unit(0.25, 'cm'), #change legend key size
        ...
        scale_fill_manual(values = dittoSeq::dittoColors(3)) +
...
#p + facet_wrap(~Location, scales = "free_x", nrow = 1)
 ...
 ggplotly(p1)
 # p1
 #  f1 <- paste("results/final-plots/taxa-rank-" , x , "_barplot_splitby_environ.pdf", sep = "")
 # ggsave(p1, filename = f1, width = 14, height = 8, device = "pdf")
})
