...
<!-- ## Grouped by Extr_Env & Split by Density -->

<!-- Since Intra- versus Interspecific doesn't seem to be as interesting, let's try grouping the data by Extr_Env and then re-plotting the composition bar plots and splitting the bar plots by Density. Here, the first 20 taxa are shown in each plot and the rest are categorized into the "Other" category. -->

<!-- ```{r, warning=FALSE} -->
<!-- #Get relative abundance -->
<!-- physeq.prev1 <- merge_samples(physeq.prev, "Extr_Env") -->
<!-- physeq.prev.prop <- transform_sample_counts(physeq.prev1, function(x) x/sum(x) ) -->
<!-- sample_data(physeq.prev.prop)$SampleSums <- sample_sums(physeq.prev1) -->
<!-- physeq.plot <- physeq.prev.prop -->
<!-- physeq.plot <- physeq.plot %>% tax_fix(unknowns = c(" unidentified"," uncultured", " Clostridiaceae_bacterium", " Clostridium_sp.", " uncultured_bacterium", " uncultured_prokaryote", " unidentified_eubacterium")) -->
...
<!--  p1 -->
<!--  f1 <- paste("../results/final-plots/taxa-rank-" , x , "_barplot_top20_groupedby_Extr_Env_splitby_Density.pdf", sep = "") -->
<!--  ggsave(f1, device = "pdf") -->
<!-- }) -->
